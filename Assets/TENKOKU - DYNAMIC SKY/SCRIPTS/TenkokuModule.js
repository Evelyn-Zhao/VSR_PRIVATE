...
var atmosphereDensity : float = 0.55;
var horizonDensity : float = 2.0;
var horizonDensityHeight : float = 0.34;
private var atmosTime : float = 1.0;
private var setAtmosphereDensity : float = 0.0;
...
cloudOverRot = Mathf.Clamp(cloudOverRot,0.0,1.0);
    ...
    if (renderObjectCloudPlane != null){
		...
		cloudSpeeds.a = 2.0 * cloudOverRot; //overcast
		...
		if (useCameraCam != null){
			//orig
			cloudPlaneObject.transform.localScale = Vector3(0.5,0.5,0.1);
			cloudPlaneObject.transform.localPosition.y = -1.0;
			cloudSc = weather_cloudScale;

			//scaled
			//cloudSz = 1.0+(30000/2000.0) * 0.2; //30000 = useCameraCam.farClipPlane
				//cloudSz = 1.0+(useCameraCam.farClipPlane/30000); //30000 = useCameraCam.farClipPlane
			//cloudPlaneObject.transform.localScale = Vector3(cloudSz,cloudSz,1);
			//cloudPlaneObject.transform.localPosition.y = -10.0;
			//cloudSc = weather_cloudScale * (1.0+(30000/2000.0)) * 0.08;
		}
   		...
   		if (useCameraCam != null && renderObjectCloudPlane != null){
   			renderObjectCloudPlane.sharedMaterial.SetFloat("_TenkokuDist",1200);
   		}
   	}
	...
	atmosTime = Vector3.Angle((sunObject.transform.position - skyObject.transform.position),skyObject.transform.up) / 180.0;
	atmosTime = Mathf.Clamp(Mathf.Lerp(-0.75,1.75,(atmosTime)),0.0,0.5);
	//setAtmosphereDensity = atmosphereDensity + (horizonDensity * 2.0 * atmosTime);
	setAtmosphereDensity = (horizonDensity * 2.0 * atmosTime);
	//Shader.SetGlobalFloat("_Tenkoku_AtmosphereDensity",floatRound(setAtmosphereDensity));
	...
	if (RenderSettings.skybox != null){
		RenderSettings.skybox.SetFloat("_TenkokuColorFac",Mathf.Lerp(1.0,1.0,weather_OvercastAmt));
		RenderSettings.skybox.SetFloat("_TenkokuExposureFac",Mathf.Lerp(1.0,1.0,weather_OvercastAmt));
	...
	useSkyBright = useSkyBright * Mathf.Lerp(1.0,10.0,ambientGI.r);
	...
	Shader.SetGlobalFloat("_Tenkoku_AmbientGI",ambientCloudCol.r);
	...
	lightObjectWorld.intensity *= Mathf.Lerp(1.0,0.8,useOvercast);

   	lightObjectWorld.shadowStrength = Mathf.Lerp(1.0,0.25,weather_OvercastAmt);
    ...
    if (useCamera != null){
		...
		rayCol = Color.Lerp(rayCol,Color(rayCol.r,rayCol.r,rayCol.r,rayCol.a),Mathf.Clamp(weather_OvercastAmt*2,0.0,1.0));
		...
		lightVals += ","+setSun;
		lightVals += ","+Mathf.Lerp(0.0,20.0,sunRayIntensity).ToString("#.0000");
...
Shader.SetGlobalColor("_Tenkoku_Daylight",lightObjectWorld.color * (lightObjectWorld.intensity*0.5) * (calcTime));
Shader.SetGlobalColor("_Tenkoku_Nightlight",lightObjectNight.color * (lightObjectNight.intensity + nightBrightness) * (1.0-calcTime) * 2);
...
function DecodeColorKey(position : String) : Color{
	...
	if (position == "sunray"){
		returnColor.r = Mathf.Pow(returnColor.r,3.2);
		returnColor.g = Mathf.Pow(returnColor.g,3.2);
		returnColor.b = Mathf.Pow(returnColor.b,3.2);
	}
