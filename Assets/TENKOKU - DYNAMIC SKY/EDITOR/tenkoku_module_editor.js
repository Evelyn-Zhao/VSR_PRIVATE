...
class tenkoku_module_editor extends Editor {
	...
	var logoTex : Texture;
	var divTex : Texture;
	var divRevTex : Texture;
	var divVertTex : Texture;
	...
	//var showUnder : boolean = false;
 	...
 	function OnEnable(){
	    ...
	    //divVertTex  = Resources.Load("textures/gui_tex_tenkoku_divvert");
 	}
	...
	function OnInspectorGUI() {
        ...
        if (target.useSunRays) spaceAdjust += 40;
			
        if (target.showConfigAtmos){
        	EditorGUI.DrawPreviewTexture(Rect(rt.x+margin,rt.y+200+spaceAdjust,387,34),divRevTex);
			GUILayout.Space(10.0);
        	...
        	GUI.Label (Rect (rt.x+margin+10, rt.y+135, 180, 15), GUIContent("Atmospheric Density"));
        	target.atmosphereDensity = EditorGUI.Slider(Rect(rt.x+margin+165, rt.y+135, setWidth, 18), "", target.atmosphereDensity,0.0,4.0);

        	GUI.Label (Rect (rt.x+margin+10, rt.y+155, 180, 15), GUIContent("Horizon Density"));
        	target.horizonDensity = EditorGUI.Slider(Rect(rt.x+margin+165, rt.y+155, setWidth, 18), "", target.horizonDensity,0.0,6.0);

        	GUI.Label (Rect (rt.x+margin+10, rt.y+175, 180, 15), GUIContent("Horizon Height"));
        	target.horizonDensityHeight = EditorGUI.Slider(Rect(rt.x+margin+165, rt.y+175, setWidth, 18), "", target.horizonDensityHeight,0.0,1.0);



			GUI.Label (Rect (rt.x+margin+10, rt.y+205, 150, 15), GUIContent ("Calculate Light Rays"));
			target.useSunRays = EditorGUI.Toggle(Rect(rt.x+margin+165, rt.y+205, 60, 15), "", target.useSunRays);
			
			if (target.useSunRays){
				GUI.Label (Rect (rt.x+margin+10, rt.y+225, 150, 15), GUIContent ("Sun Ray Intensity"));
        		target.sunRayIntensity = EditorGUI.Slider(Rect(rt.x+margin+165, rt.y+225, setWidth, 18), "", target.sunRayIntensity,0.0,1.0);

        		GUI.Label (Rect (rt.x+margin+10, rt.y+245, 150, 15), GUIContent ("Sun Ray Length"));
        		target.sunRayLength = EditorGUI.Slider(Rect(rt.x+margin+165, rt.y+245, setWidth, 18), "", target.sunRayLength,0.0,1.0);
			}

        	GUILayout.Space(200.0 + spaceAdjust);
        }
        GUILayout.Space(10.0);
